...
- [ ] (2026-01-26 JST) ショートカット表示機能のオン/オフをグローバルホットキーで切り替えられるようにする（デフォルトON、状態は `chrome.storage` に保存、Options からも切替可）。ホットキーは mac: ⌘⌥B / win・linux: Ctrl+Alt+B。Content Script でトグル状態に応じて注釈の付与/非表示を制御する。
...
- Decision: バッジ表示は CSS 擬似要素ではなく、文字単位の要素を明示的に追加して枠を描く方式を維持する。
  Rationale: 擬似要素では文字ごとの枠付けや記号混在時の柔軟なレイアウト制御が難しく、既存の per-character span 方式の方が視認性と制御性が高いため。
...
- Decision: ショートカットバッジの表示オン/オフトグル用グローバルホットキーを mac: ⌘+⌥+B、win/linux: Ctrl+Alt+B に統一する。
  Rationale: 「Badge」に紐づく B を使いつつ、ブラウザや GitHub 既存ショートカットと衝突しにくい 3 キー構成とするため。
  Date/Author: 2026-01-26 JST / assistant
