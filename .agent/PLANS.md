...
- [ ] (2026-01-27 JST) `annotate` がホットキーを見つけられない要素にも `aria-label` / `data-tooltip` を空文字で上書きしてしまう回帰を修正する。整形結果が空の場合は既存属性を保持し、値がある場合のみ追記するようロジックを戻し、空ラベルを書かないことを確認するテストを `apps/github-shortcut-badges/test/annotate.test.ts` に追加する。
- [ ] (2026-01-27 JST) トグル OFF（`ghskEnabled=false`）時にも既存バッジが表示されたままになる問題を解消する。無効化時はバッジ/ポップアップを確実に非表示または除去する CSS/DOM 処理を追加し、Popup/グローバルホットキー/Options 経由の切替で表示が消えることを `apps/github-shortcut-badges/test/contentScript.test.ts` などで検証する。
