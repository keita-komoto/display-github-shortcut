...
- [x] (2026-02-28 JST) 失敗する再現テストを先に追加した。`apps/github-shortcut-badges/test/annotate.test.ts` に「ナビメニュー文脈の要素には `data-ghsk-layer="nav"` が付与される」「無効化時に当該属性が除去される」を追加し、実装前に失敗（2件）を確認した。
- [x] (2026-02-28 JST) レイヤー制御を実装した。`apps/github-shortcut-badges/src/content/annotate.ts` にナビメニュー文脈判定（`isNavContextElement`）を追加し、`data-ghsk-layer="nav"` の付与・解除を注釈処理へ統合した。無効化時の全体クリアに `data-ghsk-layer` 除去を追加した。
- [x] (2026-02-28 JST) `apps/github-shortcut-badges/src/public/content.css` に前面表示用レイヤースタイルを追加した。`[data-ghsk-layer="nav"]` のみ z-index を引き上げ、通常要素への影響を避けた。
- [x] (2026-02-28 JST) テストと CI で回帰を確認した。`pnpm --filter github-shortcut-badges run test -- test/annotate.test.ts`、`pnpm --filter github-shortcut-badges run test`、`./ci.sh` が成功し、既存機能の非回帰を確認した。
- [x] (2026-02-28 JST) 「下表示を維持したまま隠れない」要求に合わせて CSS を再調整した。ナビ文脈のバッジは絶対配置（下表示）へ戻し、`[data-ghsk-layer="nav"]` の z-index 強化と、ナビ系祖先コンテナ（`header[role="banner"]` / `.UnderlineNav` / `.AppHeader-localBar` など）への `overflow: visible`・z-index 調整を追加した。
- [x] (2026-02-28 JST) 実DOMで `overflow: hidden auto` の祖先が残るケースに対応するため、`annotate.ts` でナビ文脈の祖先へ `data-ghsk-nav-ancestor="1"` を付与する実装に拡張した。CSS はこの属性に対して `overflow: visible` を適用する方式へ変更し、セレクタ当て打ち依存を減らした。
- [ ] (2026-02-28 JST) 手動検証を行う。GitHub のリポジトリトップと Issues 画面で、ナビメニュー直下のバッジがメインコンテンツより前面に表示され続けることを目視で確認する。（この実行環境では未実施）
...
- Decision: ナビ文脈判定は `closest` によるコンテナ検出（`.AppHeader-localBar` / `.UnderlineNav` / `.subnav-links` など）と Code 操作要素の補助判定を組み合わせ、`annotateElement` 内で都度 `data-ghsk-layer` を更新する。
  Rationale: 注釈済み要素でも再注釈時に文脈が変化し得るため、バッジ追加有無ではなく注釈処理ごとにレイヤー属性を再評価した方が残留属性を防げるため。
  Date/Author: 2026-02-28 JST / assistant
- Decision: ナビ文脈の隠れ対策は、固定セレクタだけで祖先 `overflow` を開放するのではなく、注釈時に実際の祖先チェーンへ `data-ghsk-nav-ancestor` を付与して CSS を適用する方式を採用する。
  Rationale: GitHub 側 DOM 構造が画面や AB テストで変化し、当て打ちセレクタのみでは `overflow: hidden auto` を取りこぼすため。実祖先へのマーキングで回避範囲を安定化できるため。
  Date/Author: 2026-02-28 JST / assistant
...
- Observation: `summary[aria-label="Code"]` は文脈次第で通常コンテンツにも存在し得るため、単純な要素名一致だけでは過剰前面化になる。`closest` でナビ系コンテナを判定し、ヘッダー系祖先を併用する必要があった。
  Evidence: 失敗テスト追加直後に `data-ghsk-layer` 未付与で 2 件失敗し、`isNavContextElement` 導入後に `pnpm --filter github-shortcut-badges run test -- test/annotate.test.ts` が成功。
- Observation: z-index 引き上げだけでは、GitHub ナビの祖先コンテナ側 `overflow` によるクリップが残る。表示位置は従来どおり下側（absolute）を維持し、ナビ系祖先の `overflow`/z-index も併せて調整する必要があった。
  Evidence: `apps/github-shortcut-badges/src/public/content.css` で `[data-ghsk-layer="nav"] .ghsk-badge` を absolute のまま維持しつつ、`header[role="banner"]` / `.UnderlineNav` / `.AppHeader-localBar` などに `overflow: visible` を追加後、`pnpm --filter github-shortcut-badges run test` と `./ci.sh` が成功。
- Observation: 実画面の DevTools で対象祖先の `overflow` が `hidden auto`、`z-index` が `auto` になっており、視覚的な非表示は stacking だけでなくクリップが支配的だった。
  Evidence: ユーザー提供スクリーンショットの Console 出力（`getComputedStyle($0).overflow -> hidden auto`）に基づき、`data-ghsk-nav-ancestor` 導入後も `./ci.sh` 成功を確認。


### Outcomes & Retrospective


- `annotate` にナビ文脈レイヤー制御を追加し、`data-ghsk-layer="nav"` による限定的な前面化を実装した。無効化時の全体クリアにも同属性除去を追加し、残留を防止した。
- `content.css` ではナビ文脈属性付き要素とそのバッジ・擬似ポップアップのみ z-index を上げ、全体副作用を抑制した。
- 追加調整として、ナビ文脈のバッジは下表示（absolute）を維持し、隠れ対策はナビ要素および祖先コンテナの `overflow: visible` と z-index で吸収する構成に更新した。
- さらに、隠れの再発を抑えるため `annotate` で実祖先マーキング（`data-ghsk-nav-ancestor`）を導入し、CSS を属性ベース適用へ移行した。テストは 26 件に拡張し、無効化時の祖先属性除去も自動検証対象に含めた。
- 自動検証は完了（`pnpm --filter github-shortcut-badges run test` と `./ci.sh` 成功）。実ブラウザでの手動目視検証のみ未実施のため、最終受け入れ時に別途確認が必要。
